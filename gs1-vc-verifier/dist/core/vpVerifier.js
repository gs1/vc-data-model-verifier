...
async function verifyVP(presentationJwtOrObj) {
    ...
    let vpResult = undefined;
    let vpValidationError = undefined;
    ...
    try {
        ...
        try {
            if (typeof presentationJwtOrObj === 'string') {
                const { payload } = (0, input_1.splitJwt)(presentationJwtOrObj);
    ...
    if (vpResult.content?.verifiableCredential) {
        for (const [index, vc] of vpResult.content.verifiableCredential.entries()) {
            ...
            if (vcType === 'application/vc-ld+jwt') {
                revoked = await (0, revocation_1.checkRevocationStatus)(transmute.text.decoder.decode(vcContent));
    ...
    const validatorRequest = {
        ...
        gs1DocumentResolver: {
            ...
            externalCredentialVerification: async (credential) => {
    ...
    const gs1Result = await (0, vc_verifier_rules_1.checkGS1CredentialPresentationValidation)(validatorRequest, { verifiableCredential: extractedCredentials });
