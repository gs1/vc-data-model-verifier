  ...
  <script>
    ...
    async function renderSvgFromCredentialData(credential) {
      try {
        ...
        // Prefer universal KeyCredential template when applicable
        const isKeyCredentialType = (credential.credentialType || '').includes('KeyCredential') ||
          (Array.isArray(credential.rawPayload?.type) && credential.rawPayload.type.includes('KeyCredential'));
        if (isKeyCredentialType) {
          svgUrl = 'https://gs1.github.io/GS1DigitalLicenses/templates/gs1-sample-key-template.svg';
          console.log('Using universal KeyCredential template URL:', svgUrl);
        } else if (credential.renderMethod && credential.renderMethod[0] && credential.renderMethod[0].id) {
          ...
          if (credentialType.includes('KeyCredential')) {
            svgUrl = 'https://gs1.github.io/GS1DigitalLicenses/templates/gs1-sample-key-template.svg';
          } else if (credentialType.includes('PrefixLicense')) {
        ...
        // KeyCredential-specific placeholders
        const isKeyCredential = (credential.credentialType || '').includes('KeyCredential') ||
          (Array.isArray(credential.rawPayload?.type) && credential.rawPayload.type.includes('KeyCredential'));
        if (isKeyCredential) {
          const subjectId = credential.credentialSubject?.id || '';
          const subjectIdLast = (() => {
            try {
              const lastSeg = String(subjectId).split('/').filter(Boolean).pop() || '';
              const digits = lastSeg.replace(/[^0-9]/g, '');
              return digits || lastSeg || subjectId;
            } catch { return subjectId; }
          })();
          replacements['[DIGITAL LINK]'] = subjectId;
          replacements['[KEY URI]'] = subjectId;
          replacements['[ID]'] = subjectId;
          replacements['[KEY]'] = subjectId;
          replacements['[credentialSubject.id]'] = subjectIdLast;
          replacements['[EXTENDS]'] = credential.credentialSubject?.extendsCredential || 'N/A';
          replacements['[EXTENDS CREDENTIAL]'] = credential.credentialSubject?.extendsCredential || 'N/A';
        }
...
async function renderSvgFromPayload(payload) {
  try {
    ...
    // Get SVG template URL
    let svgTemplateUrl = '';
    const isKeyCred = types.some(t => t.includes('KeyCredential'));
    if (isKeyCred) {
      // Always prefer universal KeyCredential template
      svgTemplateUrl = 'https://gs1.github.io/GS1DigitalLicenses/templates/gs1-sample-key-template.svg';
    } else if (payload.renderMethod && Array.isArray(payload.renderMethod) && payload.renderMethod.length > 0) {
      // Otherwise use renderMethod if provided
      svgTemplateUrl = payload.renderMethod[0].id;
    }
    
    // Fallback to default templates based on credential type
    if (!svgTemplateUrl) {
      if (types.some(t => t.includes('KeyCredential'))) {
        svgTemplateUrl = 'https://gs1.github.io/GS1DigitalLicenses/templates/gs1-key-template.svg';
      } else if (types.some(t => t.includes('PrefixLicense'))) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        svgTemplate = createFallbackSvgTemplate({ rawPayload: payload, credentialSubject: subject, issuer: payload.issuer, name: payload.name, credentialType: types.find(t => t !== 'VerifiableCredential') || 'Credential' });
      }
    } catch (fetchError) {
      // Use fallback template instead of returning empty
      svgTemplate = createFallbackSvgTemplate({ rawPayload: payload, credentialSubject: subject, issuer: payload.issuer, name: payload.name, credentialType: types.find(t => t !== 'VerifiableCredential') || 'Credential' });
    ...
    // Only replace [VALUE] with license for non-KeyCredential templates; KeyCredential uses [VALUE] for extends
    if (!types.includes('KeyCredential')) {
    ...
    // Always replace extends placeholders when present
    const extendsAll = subject.extendsCredential || 'N/A';
    renderedSvg = renderedSvg.replace(/\[EXTENDS\]/g, extendsAll);
    renderedSvg = renderedSvg.replace(/\[EXTENDS CREDENTIAL\]/g, extendsAll);

    // KeyCredential-specific replacements
    const isKeyCredential = types.includes('KeyCredential');
    if (isKeyCredential) {
      const keyId = subject.id || '';
      const keyIdLast = (() => {
        try {
          const lastSeg = String(keyId).split('/').filter(Boolean).pop() || '';
          const digits = lastSeg.replace(/[^0-9]/g, '');
          return digits || lastSeg || keyId;
        } catch { return keyId; }
      })();
      renderedSvg = renderedSvg.replace(/\[DIGITAL LINK\]/g, keyId);
      renderedSvg = renderedSvg.replace(/\[KEY URI\]/g, keyId);
      renderedSvg = renderedSvg.replace(/\[ID\]/g, keyId);
      renderedSvg = renderedSvg.replace(/\[KEY\]/g, keyId);
      renderedSvg = renderedSvg.replace(/\[credentialSubject.id\]/g, keyIdLast);
      const extendsCred = subject.extendsCredential || 'N/A';
      renderedSvg = renderedSvg.replace(/\[EXTENDS\]/g, extendsCred);
      renderedSvg = renderedSvg.replace(/\[EXTENDS CREDENTIAL\]/g, extendsCred);
      // Some KeyCredential templates use [VALUE] for extends
      renderedSvg = renderedSvg.replace(/\[VALUE\]/g, extendsCred);
      // Identification key type from types array (excluding 'VerifiableCredential')
      const idKeyType = types.find(t => t !== 'VerifiableCredential') || 'N/A';
      renderedSvg = renderedSvg.replace(/\[IDENTIFICATION KEY TYPE\]/g, idKeyType);
      renderedSvg = renderedSvg.replace(/\[KEY TYPE\]/g, idKeyType);
    }
    ...
    const headerLeft = `<div class="card card-compact">
      ...
      <div style="margin-top: 16px;">
        <div class="check-item">
          <span><strong style="color: #000;">Signature:</strong> ${signatureVerified ? '<span class="ok">Verified <img src="/static/check-mark.png" alt="Verified" style="width: 16px; height: 16px; vertical-align: middle;"></span>' : '<span class="bad">Not Verified</span>'}</span>
          ${joseFallbackUsed ? '<div style="font-size: 12px; color: #666; margin-top: 4px;">(using jose fallback)</div>' : ''}
        </div>
        <div class="check-item">
          <span><strong style="color: #000;">Schema:</strong> ${schemaValid ? '<span class="ok">Validated <img src="/static/check-mark.png" alt="Validated" style="width: 16px; height: 16px; vertical-align: middle;"></span>' : '<span class="bad">Not Validated </span>'}</span>
        </div>
        <div class="check-item">
        ...
        <div class="check-item">
          <span><strong style="color: #000;">GS1 rules:</strong> ${gs1Ok ? '<span class="ok">Validated <img src="/static/check-mark.png" alt="Validated" style="width: 16px; height: 16px; vertical-align: middle;"></span>' : '<span class="bad">NoT Validated</span>'}</span>
        </div>
