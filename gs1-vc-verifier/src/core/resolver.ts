    ...
    resolve: async (req: ResolverRequest): Promise<ResolverResponse> => {
      ...
      if ((type === "application/vp-ld+jwt" || type === "application/vc-ld+jwt") && content) {
        ...
        try {
          ...
          if (did.startsWith('did:')) {
            ...
            verificationMethod = didDocument.didDocument?.verificationMethod?.find(
              (vm: any) => vm.id === kid
            );
          } else {
          ...
          if (payload.issuer && payload.issuer.id && payload.issuer.id.startsWith('did:')) {
            ...
            try {
              ...
              didDocument.didDocument?.verificationMethod?.forEach((vm: any, index: number) => {
              ...
              if (!verificationMethod) {
                ...
                verificationMethod = didDocument.didDocument?.verificationMethod?.find(
                  (vm: any) => vm.publicKeyJwk?.kid === kid
                );
              }
      ...
      if (type === "application/schema+json" && id) {
        ...
        try {
          ...
          const relaxed = { ...originalSchema } as any;
          try {
      ...
      if ((type === "application/vc-ld+jwt" || type === "application/vp-ld+jwt") && id && !content) {
        ...
        const normalizeJwt = (rawIn: string): string => {
