...
import { Bitstring } from "../utils/statusList/Bitstring";
 ...
 */
async function decodeBitstring(encoded: string) {
  // Delegate to shared Bitstring utility (supports raw and gzip-compressed lists)
  return Bitstring.decodeBits({ encoded });
}
 ...
 */
export async function checkRevocationStatus(vc: any, options?: {
  assumeRevokedWhenStatusListUnavailable?: boolean;
}): Promise<boolean> {
  try {
    // Decode JWT payload
    ...
    const bitBytes = await Bitstring.decodeBits({ encoded: encodedList });
    try {
      console.log(`[Revocation] Decoded bitstring bytes=${bitBytes.length}, maxPositions=${bitBytes.length * 8}`);
    } catch (_) {}
    const bitset = new Bitstring({ buffer: bitBytes });
