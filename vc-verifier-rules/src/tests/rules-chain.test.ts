...
describe('Tests for Rules Engine Subject Field Validation', () => {
    ...
    it('should return invalid when checking organization (party) Credential chain with different subject id', async () => {
        ...
        const jsonCompanyPrefixCredential = JSON.stringify(mockPresentationParty.verifiableCredential[0]);
        const mockCompanyPrefixCredential = JSON.parse(jsonCompanyPrefixCredential);
        mockCompanyPrefixCredential.credentialSubject.id = "https://id.gs1.org/417/7360005769407";

        const resultBuildChain = await buildCredentialChain(mock_getExternalCredential, mockPresentationParty, mockCompanyPrefixCredential);
