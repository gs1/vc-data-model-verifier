import { invalidExtendedCredentialMissing, invalidIssueForPrefixLicense, invalidLicenseValueFormat, invalidLicenseValueStartPrefix, invalidIssuerNotGS1GO } from "../../engine/gs1-credential-errors.js";
...
export async function compareLicenseLengthsToExtended(credentialSubject: subjectLicenseValue | undefined, extendedCredentialSubject: subjectLicenseValue | undefined): Promise<gs1CredentialValidationRuleResult> {
    ...
    // Compare License Field Lengths - Updated for GS1 Digital Licenses Specification
    // GL-5: licenseValue MUST begin with AND MUST be at least one digit longer than the extended licenseValue
...
export async function validateExtendedLicensePrefix(credentialType: string, 
    ...
    // NEW: PL-2 - Verify Prefix License Credential Issuer is GS1 Global (did:web or did:webvh)
    const extendedCredentialIssuer = getCredentialIssuer(extendedCredential);
    if (!extendedCredentialIssuer.startsWith('did:web:') && !extendedCredentialIssuer.startsWith('did:webvh:')) {
        gs1CredentialCheck.verified = false;
        gs1CredentialCheck.errors.push(invalidIssuerNotGS1GO);
        return gs1CredentialCheck;
    }

    // Removed strict equality to a single GO DID; allowing any did:web or did:webvh issuer as per spec wording
...
// Validate the root of trust for Prefix License Credential
export function validatePrefixRootOfTrust_JsonSchema(credentialType: string, credential: VerifiableCredential): gs1RulesResult {
    ...
    if (!issuer.startsWith('did:web:') && !issuer.startsWith('did:webvh:')) {
        gs1CredentialCheck.verified = false;
        gs1CredentialCheck.errors.push(invalidIssuerNotGS1GO);
        return gs1CredentialCheck;
    }
